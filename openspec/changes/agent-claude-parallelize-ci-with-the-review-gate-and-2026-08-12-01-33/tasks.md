...
- [x] 2.1 Implement scoped behavior changes (early promote + `--gate-serial-ci`, `expectHeadSha` pin, `--review-model`, provider-bin env knobs).
- [x] 2.2 Add/update focused regression coverage (`test/review-gate.test.js`, `test/pr-review.test.js`).

## 3. Verification

- [x] 3.1 Run targeted project verification commands (`npm test` on branch vs `main`: 41 failures on both, failing set byte-identical, +17 new passing tests; `npx biome lint bin src scripts test` clean).
